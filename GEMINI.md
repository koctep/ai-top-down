...
## Role-Based Handoff Flow (Gemini CLI)
When a user provides a new task, follow this exact sequence by executing the appropriate commands:

1. **Analyst (`/analyst`)**: Gathers requirements and creates `10-requirements.md`.
2. **Product Owner (`/po`)**: Reviews `10-requirements.md` for business logic.
3. **Senior Software Engineer (`/senior`)**: Creates `20-architecture.md`.
4. **Team Lead (`/team_lead`)**: Reviews `20-architecture.md`.
5. **Junior Software Engineer (`/junior`)**: Iteratively implements code based on `20-architecture.md`.
   - *Inner Loop*: Junior writes code -> Senior reviews (`/senior`) -> Junior writes more code.
   - Once done, Junior performs code cleanup (`40-code-cleanup.md`).
6. **Senior Software Engineer (`/senior`)**: Implements observability (`50-observability.md`).
